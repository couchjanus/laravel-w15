	...
	<div class="panel panel-default">
		...
		<div class="panel-body">
			<div class="form-group" v-bind:class='[ errors.hasOwnProperty("content") ? "has-error" : "" ]'>
				...
				<span v-if='[errors.hasOwnProperty("content")]'>
					<span class="help-block" v-for="(error, index) in errors.content" :key="index">{{ error }}</span>
				</span>
...
<script>
	export default{
		...
		methods:{
			post(){
				axios.post('/api/forum',{content: this.content}).then(response => {
